...
class TestReplaceFunctional(unittest.TestCase):
    ...
    def test_replace_context_creation(self):
        ...
        # Test default values
        default_context = ReplaceContext(
            search_text="search",
...
class TestCommonEdgeCases(unittest.TestCase):
    ...
    def test_max_depth_recursion_limit(self):
        ...
        result_size = sum_folder_size(str(self.test_path), _visited_inodes=set(), _depth=101)
        result_count = sum_folder_files_count(str(self.test_path), _visited_inodes=set(), _depth=101)
    ...
    def test_symlink_cycle_detection_visited_inodes(self):
        ...
        result_size = sum_folder_size(str(self.test_path), _visited_inodes=visited_inodes, _depth=0)
        result_count = sum_folder_files_count(str(self.test_path), _visited_inodes=visited_inodes, _depth=0)
    ...
    def test_from_scandir_permission_errors(self):
        ...
        with patch('fx_bin.common.sum_folder_size', side_effect=PermissionError("Access denied")):
            with patch('fx_bin.common.sum_folder_files_count', side_effect=PermissionError("Access denied")):
        ...
        def mock_scandir(path):
            for entry in original_scandir(path):
                if entry.name == "test_link":
                    ...
                    mock_entry.is_file.side_effect = lambda follow_symlinks=True: follow_symlinks and entry.is_file()
                    mock_entry.is_dir.side_effect = lambda follow_symlinks=True: follow_symlinks and entry.is_dir()
