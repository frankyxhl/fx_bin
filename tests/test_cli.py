...
class TestReplaceCommand(unittest.TestCase):
    ...
    @patch('fx_bin.replace.main')
    def test_replace_command_with_file(self, mock_replace_main):
        ...
        with self.runner.isolated_filesystem():
            ...
            mock_replace_main.return_value = 0
            
            result = self.runner.invoke(cli, ['replace', 'old', 'new', 'test.txt'])
            self.assertEqual(result.exit_code, 0)
            mock_replace_main.assert_called_once()
            
            # Check arguments passed to replace.main
            args = mock_replace_main.call_args[0]
...
class TestJson2ExcelCommand(unittest.TestCase):
    ...
    def test_json2excel_command_with_url(self, mock_pd_main):
        ...
        result = self.runner.invoke(cli, ['json2excel', 'https://api.example.com/data', 'output.xlsx'])
        self.assertEqual(result.exit_code, 0)
