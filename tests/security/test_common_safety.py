...
class TestRecursiveDirectorySafety(unittest.TestCase):
    ...
    def test_symlink_loop_detection(self):
        ...
        self.assertLess(
            elapsed, 5.0, "Operation took too long - possible infinite loop"
        )
        ...
        if os.name != "nt":  # Skip permission test on Windows
            ...
            try:
                ...
                self.assertGreaterEqual(
                    size, 0, "Should handle permission errors gracefully"
                )
                self.assertGreaterEqual(
                    count, 0, "Should handle permission errors gracefully"
                )
    ...
    def test_large_directory_performance(self):
        ...
        # Performance checks
        self.assertLess(
            elapsed, 5.0, f"Large directory scan took too long: {elapsed:.2f}s"
        )
    ...
    def test_concurrent_directory_access(self):
        ...
        size_counts = [
            (size, count) for size, count in results if isinstance(results[0], tuple)
        ]
        if size_counts:
            expected_size, expected_count = size_counts[0]
            for size, count in size_counts[1:]:
                self.assertEqual(
                    size, expected_size, "Concurrent access gave inconsistent sizes"
                )
                self.assertEqual(
                    count, expected_count, "Concurrent access gave inconsistent counts"
                )
        ...
        with os.scandir(self.test_dir) as entries:
            for entry in entries:
                if entry.name == "test.txt":
                    ...
                    self.assertIsNone(
                        result, "Should return None for files with permission errors"
                    )
                    break
