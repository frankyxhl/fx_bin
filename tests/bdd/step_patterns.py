            ...
            if expected_format == "simple":
                ...
                for line in output.splitlines():
                    ...
                    if line and not line.startswith(('Error:', 'Warning:', 'No files')):
                        assert not re.search(r'\d{4}-\d{2}-\d{2}', line), f"Found timestamp: {line}"
                        assert not re.search(r'\d+\s*(B|KB|MB|GB)', line), f"Found size: {line}"
...
class BDDQualityValidator:
    ...
    def _check_step_clarity(self, scenario: str) -> List[str]:
        ...
        for term in vague_terms:
            if term in scenario.lower() and not re.search(rf'{term}\s+"\w+"', scenario.lower()):
    ...
    def _check_business_language(self, scenario: str) -> List[str]:
        ...
        for term in technical_terms:
            if term in scenario:
                issues.append(f"Technical term '{term}' found - consider business language")
    ...
    def _validate_given_when_then_structure(self, scenario: str) -> List[str]:
        ...
        if when_count > 1:
            issues.append("Multiple When steps found - scenario should have single action")
        
        # Check for missing Then steps
        then_count = len(re.findall(r'^\s*Then\s+', scenario, re.MULTILINE))
        
        if then_count == 0:
            issues.append("No Then steps found - scenario should have observable outcome")
...
def validate_step_definition_coverage(feature_file: Path, step_files: List[Path]) -> Dict[str, Any]:
    ...
    coverage_percentage = ((len(feature_steps) - len(undefined_steps)) / len(feature_steps)) * 100
...
class BDDDocumentationGenerator:
    ...
    def generate_feature_summary(self, feature_file: Path) -> str:
        ...
        # Extract scenarios
        scenarios = []
        for match in re.finditer(r'^\s*(@[\w-]+\s+)*Scenario:\s*(.+)', content, re.MULTILINE):
...
"""
        ...
        for tags, name in scenarios:
            priority = "🔥 Critical" if "@critical" in tags else "⚠️  Important" if "@smoke" in tags else "📋 Standard"
