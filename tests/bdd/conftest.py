...
class TestDirectory:
    ...
    path: Path
    files: List[TestFile]
    subdirectories: List['TestDirectory']
...
def directory_builder(temp_directory, file_builder):
    ...
    def create_structure(structure: Dict[str, Any]) -> TestDirectory:
        ...
        """
        def _create_recursive(spec: Dict[str, Any], base_path: Path) -> TestDirectory:
            files = []
            subdirs = []
...
def standard_test_files(file_builder):
    ...
    files['pdf'] = file_builder(
        ...
        created_offset_minutes=75
    )
    
    files['doc'] = file_builder(
        "report.docx",
        content="fake word document",
        created_offset_minutes=100
    )
...
def nested_directory_structure(directory_builder):
    ...
    structure = {
        "files": [
            {"name": "root.txt", "content": "root level file", "created_offset_minutes": 60}
        ],
        "subdirs": {
            "level1": {
                "files": [
                    {"name": "level1.txt", "content": "first level file", "created_offset_minutes": 45},
                    {"name": "level1.py", "content": "# first level python", "created_offset_minutes": 40}
                ],
                "subdirs": {
                    "level2": {
                        "files": [
                            {"name": "level2.txt", "content": "second level file", "created_offset_minutes": 30},
                            {"name": "deep.py", "content": "# deep python file", "created_offset_minutes": 25}
                        ],
                        "subdirs": {
                            "level3": {
                                "files": [
                                    {"name": "level3.txt", "content": "third level file", "created_offset_minutes": 15},
                                    {"name": "deepest.py", "content": "# deepest file", "created_offset_minutes": 10}
                                ]
                            }
                        }
                    }
            ...
            "parallel": {
                "files": [
                    {"name": "parallel.txt", "content": "parallel branch file", "created_offset_minutes": 35},
                    {"name": "side.py", "content": "# side branch python", "created_offset_minutes": 20}
                ]
            }
        }
    }
...
def files_match_extensions(files: List[TestFile], extensions: List[str]) -> bool:
...
def files_sorted_by_creation_time(files: List[TestFile], reverse: bool = False) -> bool:
...
def files_sorted_by_modification_time(files: List[TestFile], reverse: bool = False) -> bool:
