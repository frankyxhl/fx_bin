...
from .conftest import (
    ...
    files_match_extensions,
    files_sorted_by_creation_time,
...
@given(
    "the test directory contains files with different creation and modification times"
)
def setup_timestamped_files(standard_test_files):
    ...
    assert (
        len(set(modification_times)) > 1
    ), "Files should have different modification times"


@given(
    parsers.parse('I have a directory containing files with extensions "{extensions}"')
)
...
@given(
    parsers.parse(
        'I have multiple "{extension}" files with different modification times'
    )
)
...
@given(
    parsers.parse('I have multiple "{extension}" files with different creation times')
)
...
def setup_specific_files(file_builder, file_list):
    ...
    for i, name in enumerate(file_names):
        file_builder(
            name, content=f"Content for {name}", created_offset_minutes=30 + i * 5
        )
...
def run_fx_command_multiple_dirs(
    cli_runner, command_context, temp_directory, command, dir1, dir2
):
...
def verify_extension_filter(command_context, extension):
    ...
    for line in file_lines:
        ...
        if filename:
            ...
            assert (
                file_ext == ext.lower()
            ), f"Found file {filename} with extension {file_ext}, expected {ext}"
    ...
    for line in output.splitlines():
        ...
        if line and not line.startswith(("Error:", "Warning:", "No files")):
            ...
            assert not re.search(
                r"\d{4}-\d{2}-\d{2}", line
            ), f"Line contains timestamp: {line}"
            assert not re.search(
                r"\d+\s*(B|KB|MB|GB)", line
            ), f"Line contains size info: {line}"
...
def verify_detailed_format_output(command_context):
    ...
    for line in file_lines:
        ...
        assert re.search(
            r"\d{4}-\d{2}-\d{2}", line
        ), f"Line missing date format: {line}"
        # Should contain time pattern HH:MM:SS
        assert re.search(
            r"\d{2}:\d{2}:\d{2}", line
        ), f"Line missing time format: {line}"
...
def verify_sorting_newest_first(command_context, sort_type):
    ...
    assert (
        len(filenames) >= 1
    ), f"Expected multiple files for {sort_type} time sorting verification"
...
def verify_most_recent_first(command_context, time_type):
    ...
    assert (
        first_file_line is not None
    ), f"No file found in output for {time_type} verification"
...
def verify_sorting_oldest_first(command_context, sort_type):
    ...
    assert (
        len(filenames) > 0
    ), f"No files found in output for {sort_type} time sorting (oldest first)"
...
def verify_oldest_first(command_context):
    ...
    assert (
        first_file_line is not None
    ), "No file found in output for oldest first verification"
...
def verify_least_recently_modified_first(command_context):
    ...
    assert (
        first_file_line is not None
    ), "No file found for least recently modified verification"
...
def verify_single_file_output(command_context, filename):
    ...
    assert (
        filename in file_lines[0]
    ), f"Expected {filename} in output, got: {file_lines[0]}"
...
def verify_no_subdirectory_files(command_context):
    ...
    for line in output.splitlines():
        ...
        if line and not line.startswith(("Error:", "Warning:", "No files")):
            ...
            ), f"Found subdirectory file: {line}"
...
def verify_relative_paths_shown(command_context):
    ...
    assert (
        len(file_lines) >= 3
    ), f"Expected at least 3 files from recursive search, got {len(file_lines)}"
...
def verify_simple_filename_output(command_context, filename):
    ...
    for line in matching_lines:
        if not line.startswith(("Error:", "Warning:")):
            ...
            assert not re.search(
                r"\d{4}-\d{2}-\d{2}", line
            ), f"Found timestamp in: {line}"
            assert not re.search(
                r"\d+\s*(B|KB|MB|GB)", line
            ), f"Found size info in: {line}"
...
def verify_file_size_in_output(command_context):
    ...
    has_size = any(
        re.search(r"\d+\s*(B|KB|MB|GB)", line) for line in output.splitlines()
    )
...
def verify_most_recent_files(command_context, count):
    ...
    assert (
        len(file_lines) <= count
    ), f"Expected at most {count} files, got {len(file_lines)}"
...
def verify_specific_message(command_context, message):
    ...
    assert (
        message in combined_output
    ), f"Expected message '{message}' not found in output: {combined_output}"
...
def verify_error_message(command_context, message):
    ...
    assert (
        message in combined_output
    ), f"Expected error message '{message}' not found in: {combined_output}"
...
def verify_no_directory_traversal(command_context):
    ...
    output = command_context["last_output"]
    assert (
        "Error: Path not found:" in output
        or "Error: Path is not a directory:" in output
    ), "Expected path error for traversal attempt"
    ...
    for line in lines:
        ...
        if len(parts) >= 3:
            ...
            if level == "current" and path_part == "":
                ...
                file_builder(
                    str(file_path.relative_to(temp_directory)), content="test content"
                )
...
@given(
    parsers.parse('I have directories "{dir1}" and "{dir2}" with "{extension}" files')
)
def setup_multiple_directories_with_files(
    file_builder, directory_builder, dir1, dir2, extension
):
...
def setup_multiple_directories_with_files(file_builder, temp_directory, dir_list):
    ...
    for dir_name in directories:
        ...
        for i in range(2):  # Create 2 files per directory
            ...
            file_builder(
                str(file_path.relative_to(temp_directory)),
                content=f"Content in {dir_name}",
            )
...
def verify_files_from_multiple_directories(command_context):
    ...
    assert (
        len(file_lines) >= 2
    ), f"Expected files from multiple directories, got {len(file_lines)}"
...
def verify_all_three_files(command_context):
    ...
    assert (
        len(file_lines) >= 3
    ), f"Expected at least 3 files for case-insensitive matching, got {len(file_lines)}"
...
def verify_warning_message(command_context, warning):
    ...
    assert (
        warning in output
    ), f"Expected warning '{warning}' not found in output: {output}"
...
def verify_execution_time(command_context, seconds):
    ...
    ), f"Command took {execution_time:.2f}s, expected <= {seconds}s"
...
def verify_max_result_count(command_context, count):
    ...
    assert (
        len(file_lines) <= count
    ), f"Expected at most {count} results, got {len(file_lines)}"
...
def verify_python_files_size_info(command_context):
    ...
    has_size_analysis = "Size analysis:" in output or any(
        re.search(r"\d+\s*(B|KB|MB|GB)", line) for line in output.splitlines()
    )

    assert (
        has_filter_results or has_size_analysis
    ), f"Expected pipeline results with filter and size information. Output: {output}"
...
def verify_pipeline_success(command_context):
    ...
    assert (
        exit_code == 0
    ), f"Pipeline should execute successfully, got exit code {exit_code}"
...
def verify_all_three_files_shown(command_context):
    ...
    ), f"Missing extensions: {missing_extensions}. Found extensions: {found_extensions}"
...
def verify_files_from_multiple_directories(command_context):
    ...
    assert (
        len(txt_files) >= 2
    ), f"Expected files from multiple directories, got {len(txt_files)}"
...
def verify_directory_indication(command_context):
    ...
    assert (
        has_dir1_files and has_dir2_files
    ), f"Expected files from both directories. Has dir1: {has_dir1_files}, Has dir2: {has_dir2_files}"
...
def verify_options_explained(command_context):
    ...
    assert (
        found_options >= 3
    ), f"Expected multiple options explained, found {found_options}"
