...
from fx_bin import replace_functional, common_functional
...
class PerformanceBenchmarker:
    ...
    def monitor_performance(self):
        ...
        try:
            ...
            cpu_time = (
                cpu_times_end.user
                - cpu_times_start.user
                + cpu_times_end.system
                - cpu_times_start.system
            )
            cpu_percent = (
    ...
    def benchmark_function(
        ...
        if len(durations) > 2:
            ...
            percentile_95_idx = min(
                int(0.95 * len(sorted_durations)), len(sorted_durations) - 1
            )
            percentile_99_idx = min(
                int(0.99 * len(sorted_durations)), len(sorted_durations) - 1
            )
...
def run_specific_benchmarks(
    test_filter: Optional[str] = None, iterations: int = 8
) -> List[ComparisonResult]:
    """Run specific benchmarks that actually work."""
    print(
        f"Starting targeted fx_bin performance benchmarks (iterations: {iterations})...\n"
    )
    if test_filter:
    ...
    if not test_filter or test_filter == "replace":
        ...
        try:
            ...
            # Create test file
            content = (
                "This is test content with SEARCH_TEXT that needs replacement.\n" * 100
            )
            ...
            def functional_replace():
                ...
                io_result = replace_functional.work_functional(
                    "SEARCH_TEXT", "REPLACED", str(temp_file)
                )
                ...
                if hasattr(io_result, "run"):
                    ...
                    if isinstance(result, Failure):
                        raise Exception(
                            f"Functional replacement failed: {result.failure()}"
                        )
                else:
                    # io_result is already the result
                    if isinstance(io_result, Failure):
                        raise Exception(
                            f"Functional replacement failed: {io_result.failure()}"
                        )
                temp_file.unlink(missing_ok=True)
                return io_result

            imperative_result = benchmarker.benchmark_function(
                imperative_replace, "replace_imperative"
            )
            functional_result = benchmarker.benchmark_function(
                functional_replace, "replace_functional"
            )

            time_overhead = calculate_overhead(
                imperative_result.duration_ms, functional_result.duration_ms
            )
            memory_overhead = calculate_overhead(
                imperative_result.memory_peak_mb, functional_result.memory_peak_mb
            )
    ...
    # 2. Folder Size Calculation Benchmark
        ...
        try:
            ...
            def functional_size():
                ...
                try:
                    ...
                    result_context = common_functional.sum_folder_size_functional(
                        str(test_dir)
                    )
            ...
            imperative_result = benchmarker.benchmark_function(
                imperative_size, "size_imperative"
            )
            functional_result = benchmarker.benchmark_function(
                functional_size, "size_functional"
            )
    ...
    if not test_filter or test_filter == "count":
        ...
        try:
            ...
            def functional_count():
                ...
                try:
                    return common_functional.sum_folder_files_count_legacy(
                        str(test_dir)
                    )
                except Exception as e:
                    # If legacy doesn't work, try the functional version directly
                    ...
                    context = FolderContext(visited_inodes=set(), max_depth=100)
                    result_context = (
                        common_functional.sum_folder_files_count_functional(
                            str(test_dir)
                        )
                    )
            ...
            imperative_result = benchmarker.benchmark_function(
                imperative_count, "count_imperative"
            )
            functional_result = benchmarker.benchmark_function(
                functional_count, "count_functional"
            )
...
def generate_final_report(results: List[ComparisonResult]) -> str:
    ...
    report.append(
        f"Architecture: {system_info['architecture'][0]} {system_info['architecture'][1]}"
    )
    report.append("=" * 80)
    report.append("")
    ...
    # Executive Summary
    successful_results = [
        r for r in results if r.imperative.success and r.functional.success
    ]
    if successful_results:
        avg_time_overhead = sum(r.overhead_percent for r in successful_results) / len(
            successful_results
        )
        avg_memory_overhead = sum(
            r.memory_overhead_percent for r in successful_results
        ) / len(successful_results)

        report.append("EXECUTIVE SUMMARY:")
        report.append(
            f"• Benchmarks completed: {len(successful_results)}/{len(results)}"
        )
    ...
    for i, result in enumerate(results, 1):
        ...
        if result.imperative.success and result.functional.success:
            report.append(
                f"   CPU Usage - Imperative: {result.imperative.cpu_percent:.1f}%, "
                f"Functional: {result.functional.cpu_percent:.1f}%"
            )
            report.append(
                f"   Statistical Details - Imperative: std={result.imperative.std_dev_ms:.2f}ms, "
            ...
            report.append(
                f"   Statistical Details - Functional: std={result.functional.std_dev_ms:.2f}ms, "
    ...
    if successful_results:
        ...
        report.append(
            f"• Best time performance: {best_time.operation} ({best_time.overhead_percent:+.1f}%)"
        )
        report.append(
            f"• Worst time performance: {worst_time.operation} ({worst_time.overhead_percent:+.1f}%)"
        )
        report.append(
            f"• Best memory efficiency: {best_memory.operation} ({best_memory.memory_overhead_percent:+.1f}%)"
        )
        report.append(
            f"• Worst memory efficiency: {worst_memory.operation} ({worst_memory.memory_overhead_percent:+.1f}%)"
        )
        report.append("")
        ...
        medium_overhead = [
            r for r in successful_results if 15 <= r.overhead_percent < 40
        ]
    ...
    if successful_results:
        ...
        if avg_time_overhead < 20:
            report.append(
                "✓ PROCEED: Functional implementation shows excellent performance"
            )
            report.append(
                "  The safety benefits significantly outweigh the minimal overhead"
            )
        elif avg_time_overhead < 35:
            report.append(
                "⚠ CONDITIONAL: Moderate overhead but acceptable for most use cases"
            )
            report.append(
                "  Consider profiling critical paths and optimizing hot spots"
            )
        else:
        ...
        if any(r.overhead_percent > 30 for r in successful_results):
            ...
            for r in successful_results:
                if r.overhead_percent > 30:
                    report.append(
                        f"• {r.operation}: {r.overhead_percent:.1f}% overhead"
                    )
    ...
    if successful_results:
        ...
        if avg_time_overhead < 25:
            conclusion = (
                "The functional refactoring demonstrates excellent performance characteristics. "
            ...
            conclusion = (
                "The functional refactoring shows higher overhead than ideal. "
...
if __name__ == "__main__":
    try:
        ...
        results = run_specific_benchmarks(
            test_filter=args.test, iterations=args.iterations
        )
