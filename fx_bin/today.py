...
def validate_date_format(date_format: Optional[str]) -> bool:
    ...
    # Flags can be: -, _, 0, ^, #
    format_token_pattern = r"%[-_0^#]?[A-Za-z]"  # nosec B105 - not a password, regex for strftime format tokens
    ...
    if exec_shell:
        try:
            ...
            if sys.platform == "win32":
                ...
                if is_powershell:
                    os.execv(shell_cmd, [shell_cmd, "-NoLogo"])  # nosec B606 - intentional shell launch
                else:
                    # cmd.exe
                    os.execv(shell_cmd, [shell_cmd])  # nosec B606 - intentional shell launch
            else:
                # Unix-like systems
                shell_name = os.path.basename(shell_cmd)
                os.execv(shell_cmd, [shell_name])  # nosec B606 - intentional shell launch
